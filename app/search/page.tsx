              ...
              <div className="search-result__content">
                ...
                <div className="mini-offers">
                  {offers.slice(0, 3).map((offer: any) => {
                    ...
                    return <div key={offer.id} className="mini-offer"><div><b>{offer.store.name}</b>{location && <small>{location.city}{location.address ? ` · ${location.address}` : ""}</small>}<StoreHours openingHours={location?.openingHours ?? (result.mode === "demo" ? offer.store.openingHours : null)} locale={locale} t={t} />{offer.distanceKm != null && <small>{formatMessage(t.distanceAway, { distance: Number(offer.distanceKm).toFixed(1) })}</small>}</div><span className="stock-stack"><span className="stock-label">{stockLabel(status, quantity, locale)}</span>{quantity == null && <small className="availability-note">{locale === "ro" ? "verificați în magazin" : "уточняйте в магазине"}</small>}</span><strong>{Number(offer.price).toLocaleString(numberLocale(locale))} {offer.currency}</strong></div>;
                  })}
                </div>
